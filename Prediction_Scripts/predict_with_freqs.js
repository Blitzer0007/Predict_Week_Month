// predict_with_freqs.js
// Usage:
//   node predict_with_freqs.js Test_Data/weekly.xlsx Test_Data/monthly.xlsx
// Produces outputs in ./predict_with_freqs/
...
const TOP_K_PER_POS = 6;  // used for enumerating candidate combos from positional probs
...
async function produceWeeklyWorkbook(weeklyPath, outFileName) {
  ...
  buildPositionalRows(overall).forEach(r => wsPosAll.addRow(r));

  // positional_weekday
  const wsPosWeek = wb.addWorksheet('positional_weekday');
  for (let w=0; w<=6; w++) {
    wsPosWeek.addRow([`WEEKDAY_${w}`]);
    buildPositionalRows(weekCounters[w] || mkCounter()).forEach(r => wsPosWeek.addRow(r));
    wsPosWeek.addRow([]);
  }
...
async function produceMonthlyWorkbook(monthlyPath, outFileName) {
  ...
  for (let m=1; m<=12; m++) {
    ...
    buildPositionalRows(monthCounters[m] || mkCounter()).forEach(r => wsPosMonth.addRow(r));
    wsPosMonth.addRow([]);
  }
  ...
  for (const d of dates) {
    ...
    if (monthDayCounters[key] && (monthDayCounters[key].total || 0) > 0) {
      preferCounter = monthDayCounters[key]; source = 'month-day';
    } else if (monthCounters[m] && (monthCounters[m].total || 0) > 0) {
      preferCounter = monthCounters[m]; source = 'month';
    } else {
      preferCounter = overall; source = 'overall';
    }
    const chosenCounter = preferCounter || overall;
    const finalDist = buildFinalDist(preferCounter, chosenCounter, ALPHA, MIX);
    const top = topCandidatesFromDist(finalDist, TOP_N);
  ...
  try {
    ...
    const summary = {
      ...
      weekly: {
        ...
        perWeekday: (() => { const m = {}; for (let w=0;w<=6;w++) m[w] = (wkRes.weekCounters && wkRes.weekCounters[w] && wkRes.weekCounters[w].total) || 0; return m; })(),
      ...
      monthly: {
        ...
        perMonth: (() => { const mm = {}; for (let m=1;m<=12;m++) mm[m] = (mRes.monthCounters && mRes.monthCounters[m] && mRes.monthCounters[m].total) || 0; return mm; })(),
    ...
    console.log('All done. Outputs in:', outputFolder);
  } catch (err) {
